...
- [x] Deploy the exact-origin Better Auth CORS response for the approved
  ...
  `src/lib/auth-cors.test.ts`; revision `9c479a4` is live and the public
  preflight returns the exact Booking origin with credentials enabled. A real
  successful password/TOTP or SSO callback remains a consumer-side acceptance
  step and must be completed with an authorized account.
